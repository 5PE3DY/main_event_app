...
export const loader = async ({ params }) => {
  ...
  const event = await fetch(`http://localhost:3333/events/${params.eventId}`);
  const categories = await fetch("http://localhost:3333/categories");
...
export const EventPage = () => {
  ...
  const handleEditSubmit = (event) => {
    ...
    fetch(`http://localhost:3333/events/${updatedEvent.id}`, {
      method: "PUT",
      headers: {
  ...
  const handleDeleteClick = () => {
    if (window.confirm("Do you want to delete this event?")) {
      ...
      fetch(`http://localhost:3333/events/${event.id}`, {
        method: "DELETE",
      })
        .then(() => {
          toast({
